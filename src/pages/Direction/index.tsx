...
export default function HomePage() {
  ...
  const removeHandler = (id: number) => {
    request(`https://localhost:44396/Direction/${id}`, { method: 'DELETE' }).then(() => {
  ...
  React.useEffect(() => {
    setLoad(true);
    request("https://localhost:44396/Direction").then((x) => { setDatasourse(x); setLoad(false);  });
  }, []);
  ...
  const columns: ColumnsType<never> = [
    ...
    },
    {
      ...
      render: (value, record: any, index) => {
        return <>
          <div>
            <Link to={`/Direction/edit/${record.id}`}>Редактрировать </Link>
